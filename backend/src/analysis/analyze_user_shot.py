...
class UserShotAnalyzer:
    def __init__(self):
        ...
        # Use absolute paths from environment variables with fallback to base_dir
        self.models_dir = os.getenv("MODELS_DIR", str(base_dir / "data" / "models"))
        self.reference_dir = os.getenv("REFERENCE_DIR", str(base_dir / "data" / "reference" / "reference_shots"))
        self.processed_dir = os.getenv("PROCESSED_DIR", str(base_dir / "data" / "processed"))
        self.results_dir = os.getenv("RESULTS_DIR", str(base_dir / "data" / "results" / "analysis" / "analysis"))
